...
static BotChannel *botchanlist[256];

/* Stuff to pass to the command handler. */
static Agent a_BotServ;
...
void botserv_init(void) {

	#ifdef FIX_USE_MPOOL
	botdb_mempool = mempool_create(MEMPOOL_ID_BOTDB, sizeof(ChannelInfo), MP_IPB_CHANDB, MB_IBC_CHANDB);
	...
	a_BotServ.nick = s_BotServ;
	a_BotServ.shortNick = s_BS;
	a_BotServ.agentID = AGENTID_BOTSERV;
	a_BotServ.logID = logid_from_agentid(AGENTID_BOTSERV);
}

void botserv_terminate(void) {
...
void botserv(CSTR source, User *callerUser, char *buf) {

	char *cmd = strtok(buf, " ");

	TRACE_MAIN_FCLT(FACILITY_BOTSERV);

	if (IS_NULL(cmd))
		send_notice_lang_to_user(s_BotServ, callerUser, GetCallerLang(), ERROR_ACCESS_DENIED);
		...
		if (IS_EMPTY_STR(cmd))
			LOG_SNOOP(s_BotServ, "Invalid CTCP from \2%s\2", source);

		else if (str_equals_nocase(cmd, "PING")) {

			send_notice_to_user(s_BotServ, callerUser, "\001PING\001");
			LOG_SNOOP(s_BotServ, "CTCP: PING from \2%s\2", source);
		}
		else {
			...
			if (action) {
				...
				LOG_SNOOP(s_BotServ, "CTCP: %s %s from \2%s\2", cmd, action, source);
			}
			else {

				cmd[str_len(cmd) - 1] = '\0';
				LOG_SNOOP(s_BotServ, "CTCP: %s from \2%s\2", cmd, source);
			}
		}
	}
	else
		oper_invoke_agent_command(cmd, botserv_commands, callerUser, &a_BotServ);
}
...
void save_cs_dbase(void) {
	...
	if (!(f = open_db_write(s_ChanServ, CHANSERV_DB))) {
...
static BotChannel *makechan(CSTR chan) {
...
void botserv_parse_message(CSTR source, CSTR channel, CSTR message) {
	...
	if (IS_NULL(source) || IS_NULL(channel) || IS_NULL(message)) {

		log_error(FACILITY_BOTSERV_PARSE_MESSAGE, __LINE__, LOG_TYPE_ERROR_ASSERTION, LOG_SEVERITY_ERROR_HALTED, 
			"botserv_parse_message() called with invalid parameter(s): %s %s %s", source, channel, message);
	...
	if (IS_NULL(user = hash_onlineuser_find(source))) {
		...
		log_error(FACILITY_BOTSERV_PARSE_MESSAGE, __LINE__, LOG_TYPE_ERROR_ASSERTION, LOG_SEVERITY_ERROR_HALTED, 
			"botserv_parse_message(): Privmsg from nonexistent user %s to channel %s: %s", source, channel, message);

		return;
	}

	if (IS_NULL(aChannel = botserv_find(channel))) {

		log_error(FACILITY_BOTSERV_PARSE_MESSAGE, __LINE__, LOG_TYPE_ERROR_ASSERTION, LOG_SEVERITY_ERROR_HALTED, 
			"botserv_parse_message(): Privmsg from user %s to unregistered channel %s: %s", source, channel, message);

		return;
	}

	if (str_equals_partial(message, "\1ACTION ", 8))
		isAction = TRUE;

	if ((FlagUnset(aChannel->flags, BOTSERV_FLAG_DONTKICKOPS) && FlagUnset(aChannel->flags, BOTSERV_FLAG_DONTKICKVOICES)) ||
		(FlagSet(aChannel->flags, BOTSERV_FLAG_DONTKICKOPS) && !user_is_chanop(
...
static void do_register(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	else if ((reserved = check_reservedName(chan + 1, RESERVED_CHAN, 1, s_ChanServ, source, callerUser->username, callerUser->host, callerUser->ip, (IS_NOT_NULL(callerUser->oper) || user_is_ircop(callerUser) || user_is_admin(callerUser) || user_is_services_agent(callerUser)), callerUser->current_lang)) == CRNR_TOKILL)
		send_KILL(s_ChanServ, source, lang_msg(GetCallerLang(), RESERVED_NAME_KILL_REASON_REG), TRUE);

	else if (reserved == CRNR_AKILLED)
		return;

	else if (reserved == CRNR_BLOCK) {
...
static void do_identify(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if (IS_NULL(chan = strtok(NULL, " ")) || IS_NULL(pass = strtok(NULL, " "))) {
		...
		if (str_equals(pass, ci->founderpass)) {
			...
			update_invalid_password_count(callerUser, chan);
		}
	}
}
...
static void do_set_founder(User *callerUser, ChannelInfo *ci, CSTR param, CSTR accessName, const int accessMatch) {
	...
	if (IS_NULL(newFounder = findnick(param)))
		...
		/* Log this action. */
		LOG_SNOOP(s_OperServ, "CS F %s -- by %s (%s@%s) [%s -> %s] [P: %s -> %s-%lu]", ci->name, callerUser->nick, callerUser->username, callerUser->host, ci->founder, newFounder->nick, ci->founderpass, CRYPT_NETNAME, randID);
		...
		*newpass++ = 0;
		...
		if (strchr(newpass, ' '))
			...
			if (str_equals(param, ci->founderpass)) {
				...
				update_invalid_password_count(callerUser, ci->name);
			}
		} 
	}
	else {
...
static void handle_xop(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if (IS_NULL(chan = strtok(NULL, " ")) || IS_NULL(cmd = strtok(NULL, " ")) || (IS_NULL(nick = strtok(NULL, " ")) &&
		...
		str_not_equals_nocase(cmd, "LOCK") && str_not_equals_nocase(cmd, "UNLOCK"))) {
...
static void do_akick(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if ((FlagSet(ci->flags, CI_OPERONLY) && !user_is_ircop(callerUser)) ||
...
static void do_info(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if (IS_NULL(channel)) {
		...
		if ((ci->settings & CI_NOTICE_VERBOSE_RESETMASK) != CI_ACCCESS_NO_LOCK)
			send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_INFO_LOCKED_LISTS, ci->settings & CI_ACCCESS_CFOUNDER_LOCK ? "Co-Founder " : "", ci->settings & CI_ACCCESS_SOP_LOCK ? "SOP " : "", ci->settings & CI_ACCCESS_AOP_LOCK ? "AOP " : "", ci->settings & CI_ACCCESS_VOP_LOCK ? "VOP " : "", ci->settings & CI_ACCCESS_AKICK_LOCK ? "AKICK" : "");
		...
		if (isHelper) {
			...
			if (FlagSet(ci->flags, CI_OPERONLY))
				send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), OPER_CS_INFO_CHAN_OPERONLY);
...
static void do_count(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if (IS_NULL(chan)) {
		...
		if ((accessLevel < CS_ACCESS_VOP) && (!isHelper || FlagSet(ci->flags, CI_MARKCHAN) ||
			(FlagSet(ci->flags, CI_OPERONLY) && !user_is_ircop(callerUser)) ||
			(FlagSet(ci->flags, CI_SOPONLY) && !is_services_oper(callerUser)) ||
			...
			send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_COUNT_LIST, ci->name, cf, sop, aop, vop, ci->akickcount);
		}
	}
}
...
void chanserv_listchans(const User *callerUser, CSTR nick, const BOOL isSelf) {
	...
	ChannelInfo *ci;
	short		acc, accIdx;
	short		accLevels[6] = { CS_ACCESS_FOUNDER, CS_ACCESS_COFOUNDER, CS_ACCESS_SOP, CS_ACCESS_AOP, CS_ACCESS_VOP, CS_ACCESS_AKICK };
...
static void handle_op_voice(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if (IS_NULL(chan_name)) {
		...
		if ((accessLevel == CS_ACCESS_VOP) && !isOper) {
			...
			/* E' un VOP. può usere solo /CS VOICE|DEVOICE e solo su se stesso. */

			TRACE_MAIN();
			user = user_list[0];

			if ((mode != 'v') || str_not_equals_nocase(user->nick, source))
				send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), ERROR_ACCESS_DENIED);

			else if (!user_isin_chan(callerUser, chan_name) ||
				((action == '+') && user_is_chanvoice(source, chan_name, chan)) ||
				((action == '-') && !user_is_chanvoice(source, chan_name, chan)))
				send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_OP_DEOP_ERROR_NOTHING_TO_DO);

			else {

				/* ok. mettere in +|-v il chiamante. */

				TRACE_MAIN();
				send_cmd(":%s MODE %s %cv %s", s_ChanServ, chan_name, action, source);

				if (action == '+')
					chan_add_voice(chan, (User *)callerUser);
				else
					chan_remove_voice(chan, (User *)callerUser);

				if (CSMatchVerbose(ci->settings, CI_NOTICE_VERBOSE_CLEAR)) {

					if (accessMatch)
						send_cmd(lang_msg(EXTRACT_LANG_ID(ci->langID), CS_VERBOSE_OPNOTICE_OP_DEOP_USED), s_ChanServ, ci->name, source, action == '+' ? "VOICE" : "DEVOICE", source);
					else
						send_cmd(lang_msg(EXTRACT_LANG_ID(ci->langID), CS_VERBOSE_OPNOTICE_OP_DEOP_USED_THROUGH), s_ChanServ, ci->name, source, accessName, action == '+' ? "VOICE" : "DEVOICE", source);
				}

				if (accessMatch)
					log_services(LOG_SERVICES_CHANSERV_ACCESS, "OP/VOICE %s %cv %s -- by %s (%s@%s)", ci->name, action, source, source, callerUser->username, callerUser->host);
				else
					log_services(LOG_SERVICES_CHANSERV_ACCESS, "OP/VOICE %s %cv %s -- by %s (%s@%s) through %s", ci->name, action, source, source, callerUser->username, callerUser->host, accessName);
			}
		}
		else if ((accessLevel >= CS_ACCESS_AOP) || isOper) {

			//	E' un AOP o superiore, o un SA+.
			//	Puo' mettere in +|-v o +|-o chi gli pare (OpGuard e Protect permettendo).

			char nick_list_buf[((NICKMAX + 1) * USER_MAX_MODES) + 1];
			char globops_buf[((NICKMAX + 1) * USER_MAX_MODES) + 1];
			char modes[SERVER_MAX_MODES + 2], target_name[NICKSIZE];
			int skipIdx, modeIndex = 1, neverop = 0, opGuard = 0, protect = 0;
			int target_level, target_match;
			...
			for (idx = 0; idx < user_count; ++idx) {
				...
				TRACE_MAIN();
				for (skipIdx = idx+1; skipIdx < user_count; ++skipIdx) {

					if (user_list[skipIdx] == user_list[idx]) {
				...
				switch (mode) {
					...
					case 'o':
						if (action == '+' ? user_is_chanop(user->nick, ci->name, chan) : !user_is_chanop(user->nick, ci->name, chan))
							continue;

						break;

					case 'v':
						if (action == '+' ? user_is_chanvoice(user->nick, ci->name, chan) : !user_is_chanvoice(user->nick, ci->name, chan))
							continue;

						break;
				}

				if (!user_isin_chan(user, chan_name))
					continue;

				if (action == '+' && mode == 'o' && user->ni &&
					FlagSet(user->ni->flags, NI_NEVEROP) && !isOper &&
					str_not_equals_nocase(source, user->nick)) {

					send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_OP_DEOP_ERROR_NEVEROP_ON, user->nick);
					++neverop;
					continue;
				}

				target_level = get_access(user, ci, target_name, &target_match, NULL);

				/* arrivati qui il chiamante e' sicuramente autorizzato ad usare questo comando */

				if ((action == '-') && (mode == 'o') && FlagSet(ci->flags, CI_PROTECTED) && (target_level >= CS_ACCESS_AOP)
					&& (accessLevel < CS_ACCESS_COFOUNDER) && (user != callerUser)) {

					send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_OP_DEOP_ERROR_PROTECT_ON, user->nick);
					++protect;
				}
				else if ((action != '+') || /* E' un deop/devoice. */
					isOper || /* Il chiamante e' SA o +z. */
					(target_level >= CS_ACCESS_AOP) || /* Il nick da oppare e' >= AOP. */
					(accessLevel >= ((FlagSet(ci->flags, CI_OPGUARD) && (mode != 'v')) ? CS_ACCESS_COFOUNDER : CS_ACCESS_AOP)) /* Se l'OpGuard e' attiva il chiamante deve essere CF o Founder, altrimenti basta AOP. */
					) {
					...
					if (action == '+') {
						...
						if (mode == 'o') {
							...
							if ((accessLevel < CS_ACCESS_AOP) || (FlagSet(ci->flags, CI_OPGUARD)
								&& (accessLevel < CS_ACCESS_COFOUNDER) && (target_level < CS_ACCESS_AOP))) {

								/* Se non e' almeno AOP, o almeno CF se c'e' l'OpGuard attivo,
								   non ha accesso, a meno che l'utente da oppare non sia almeno AOP. */

								if (*globops_buf)
									strcat(globops_buf, "\2, \2");
								strcat(globops_buf, user->nick);
							}
							...
							if ((accessLevel < CS_ACCESS_VOP) || ((accessLevel == CS_ACCESS_VOP)
								&& str_not_equals_nocase(user->nick, callerUser->nick))) {

								/* Se non e' almeno VOP o non sta dando il +v a se' stesso, non ha accesso. */
						...
						if (mode == 'o') {
							...
							if ((accessLevel < CS_ACCESS_AOP) || (FlagSet(ci->flags, CI_PROTECTED)
								&& (accessLevel < CS_ACCESS_COFOUNDER) && (target_level >= CS_ACCESS_AOP)
								&& (user != callerUser))) {

								/* Se non e' almeno AOP, o almeno CF se c'e' il Protect attivo, non ha accesso,
								   a meno che l'utente da deoppare non sia inferiore ad un AOP, o se' stesso. */
							...
							if ((accessLevel < CS_ACCESS_VOP) || ((accessLevel == CS_ACCESS_VOP)
								...
								/* Se non e' almeno VOP o non sta togliendo il +v a se' stesso, non ha accesso. */
					...
					strcat(nick_list_buf, user->nick);

					if (target_level >= CS_ACCESS_AOP)
						ci->last_used = NOW;
				}
				else {

					send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_OP_DEOP_ERROR_OPGUARD_ON, user->nick);
					++opGuard;
				}
			}
			...
			if (*nick_list_buf) {
				...
				if (*globops_buf) {
					...
					if (accessMatch)
						send_globops(s_ChanServ, "\2%s\2 %s \2%s\2 on \2%s\2", source, ((mode == 'v') ? ((action == '+') ? "voiced" : "devoiced") : ((action == '+') ? "opped" : "deopped")), globops_buf, ci->name);
					else
						send_globops(s_ChanServ, "\2%s\2 (through \2%s\2) %s \2%s\2 on \2%s\2", source, accessName, ((mode == 'v') ? ((action == '+') ? "voiced" : "devoiced") : ((action == '+') ? "opped" : "deopped")), globops_buf, ci->name);
				}

				send_cmd(":%s MODE %s %s %s", s_ChanServ, chan_name, modes, nick_list_buf); 

				if (CSMatchVerbose(ci->settings, CI_NOTICE_VERBOSE_CLEAR)) {

					if (accessMatch)
						send_cmd(lang_msg(EXTRACT_LANG_ID(ci->langID), CS_VERBOSE_OPNOTICE_OP_DEOP_USED), s_ChanServ, ci->name, source, data->commandName, nick_list_buf);
					else
						send_cmd(lang_msg(EXTRACT_LANG_ID(ci->langID), CS_VERBOSE_OPNOTICE_OP_DEOP_USED_THROUGH), s_ChanServ, ci->name, source, accessName, data->commandName, nick_list_buf);
				}

				if (accessMatch)
					log_services(LOG_SERVICES_CHANSERV_ACCESS, "OP/VOICE %s %s %s -- by %s (%s@%s)", ci->name, modes, nick_list_buf, source, callerUser->username, callerUser->host);
				else
					log_services(LOG_SERVICES_CHANSERV_ACCESS, "OP/VOICE %s %s %s -- by %s (%s@%s) through %s", ci->name, modes, nick_list_buf, source, callerUser->username, callerUser->host, accessName);
			}
			else if (!neverop && (user_count - (opGuard + protect) > 0))
				send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_OP_DEOP_ERROR_NOTHING_TO_DO);
		}
		else
			send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), ERROR_ACCESS_DENIED);
			...
			if (anAccess->level >= accessLevel)
				...
				if (CSMatchVerbose(ci->settings, CI_NOTICE_VERBOSE_ACCESS)) {
					...
					if (accessMatch)
						...
						send_cmd(lang_msg(EXTRACT_LANG_ID(ci->langID), (anAccess->status == ACCESS_ENTRY_EXPIRED) ? CS_VERBOSE_OPNOTICE_XOP_ADDED : (((listLevel > oldLevel) ? CS_VERBOSE_OPNOTICE_XOP_RAISED_THROUGH : CS_VERBOSE_OPNOTICE_XOP_DEMOTED_THROUGH))), s_ChanServ, ci->name, source, accessName, anAccess->name, listName);
				}
...
static void do_level(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	if (FlagSet(ci->flags, CI_OPERONLY)) {
		...
		channelLevel = CI_OPERONLY;
		canChange = CheckOperAccess(data->userLevel, CMDLEVEL_SOP);
	}
	else if (FlagSet(ci->flags, CI_SOPONLY)) {
	...
	if (str_equals_nocase(who, "OPERS")) {

		TRACE_MAIN();
		RemoveFlag(ci->flags, CI_SOPONLY | CI_SAONLY | CI_SRAONLY | CI_CODERONLY);
		AddFlag(ci->flags, CI_OPERONLY);

		if (data->operMatch) {

			LOG_SNOOP(s_OperServ, "CS +L %s -- by %s (%s@%s) [Opers]", ci->name, callerUser->nick, callerUser->username, callerUser->host);
			log_services(LOG_SERVICES_CHANSERV_GENERAL, "+L %s -- by %s (%s@%s) [Opers]", ci->name, callerUser->nick, callerUser->username, callerUser->host);

			send_globops(s_ChanServ, "\2%s\2 set LEVEL for \2%s\2 to: \2Opers\2", source, ci->name);
		}
		else {

			LOG_SNOOP(s_OperServ, "CS +L %s -- by %s (%s@%s) through %s [Opers]", ci->name, callerUser->nick, callerUser->username, callerUser->host, data->operName);
			log_services(LOG_SERVICES_CHANSERV_GENERAL, "+L %s -- by %s (%s@%s) through %s [Opers]", ci->name, callerUser->nick, callerUser->username, callerUser->host, data->operName);

			send_globops(s_ChanServ, "\2%s\2 (through \2%s\2) set LEVEL for \2%s\2 to: \2Opers\2", source, data->operName, ci->name);
		}

		send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), CS_LEVEL_LEVEL_CHANGED, ci->name, "Opers");

		if (CONF_SET_READONLY)
			send_notice_lang_to_user(s_ChanServ, callerUser, GetCallerLang(), WARNING_READONLY);
	}
	else if (str_equals_nocase(who, "SOP")) {

		TRACE_MAIN();
		RemoveFlag(ci->flags, CI_OPERONLY | CI_SAONLY | CI_SRAONLY | CI_CODERONLY);
		...
		TRACE_MAIN();
		RemoveFlag(ci->flags, CI_OPERONLY | CI_SOPONLY | CI_SRAONLY | CI_CODERONLY);
		...
		TRACE_MAIN();
		RemoveFlag(ci->flags, CI_OPERONLY | CI_SOPONLY | CI_SAONLY | CI_CODERONLY);
		...
		TRACE_MAIN();
		RemoveFlag(ci->flags, CI_OPERONLY | CI_SOPONLY | CI_SAONLY | CI_SRAONLY);
		...
		TRACE_MAIN();
		RemoveFlag(ci->flags, CI_OPERONLY | CI_SOPONLY | CI_SAONLY | CI_SRAONLY | CI_CODERONLY);
	...
	if (str_equals_nocase(command, "FOUNDER")) {
		...
		if (IS_NULL(new_founder)) {
			...
			if (data->operMatch) {
				...
				LOG_SNOOP(s_OperServ, "CS T %s -- by %s (%s@%s) [F: %s -> %s] [P: %s -> %s-%lu]", ci->name, callerUser->nick, callerUser->username, callerUser->host, ci->founder, ni->nick, ci->founderpass, CRYPT_NETNAME, randID);
				...
				LOG_SNOOP(s_OperServ, "CS T %s -- by %s (%s@%s) through %s [F: %s -> %s] [P: %s -> %s-%lu]", ci->name, callerUser->nick, callerUser->username, callerUser->host, data->operName, ci->founder, ni->nick, ci->founderpass, CRYPT_NETNAME, randID);
			...
			for (anAccess = ci->access, idx = 0; (idx < ci->accesscount) && IS_NOT_NULL(anAccess); ++anAccess, ++idx) {
				...
				if ((anAccess->status == ACCESS_ENTRY_NICK) && str_equals_nocase(new_founder, anAccess->name)) {
					...
					send_notice_to_user(s_ChanServ, callerUser, "\2%s\2 was removed from the %s list.", anAccess->name, (anAccess->level == CS_ACCESS_COFOUNDER) ? "Co-Founder" : ((anAccess->level == CS_ACCESS_SOP) ? "SOP" : ((anAccess->level == CS_ACCESS_AOP) ? "AOP" : "VOP")));
