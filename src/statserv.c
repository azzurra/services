...
static void do_login(const char *source, User *callerUser, ServiceCommandData *data);
...
static void do_stats(const char *source, User *callerUser, ServiceCommandData *data);
...
// 'P' (80 / 15)
// 'Q' (81 / 16)
static ServiceCommand	statserv_commands_Q[] = {
...
// 'T' (84 / 19)
// 'U' (85 / 20)
static ServiceCommand	statserv_commands_U[] = {
...
ServiceCommand	*statserv_commands[26] = {
	...
	statserv_commands_Q,	statserv_commands_R,
	statserv_commands_S,	NULL,
	statserv_commands_U,	statserv_commands_V,
...
static void do_login(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	TRACE_MAIN_FCLT(FACILITY_STATSERV_HANDLE_LOGIN);

	if (IS_NULL(nick = strtok(NULL, " "))) {

		send_notice_to_user(s_StatServ, callerUser, "Syntax: \2LOGIN\2 [nick] password)");
		send_notice_to_user(s_StatServ, callerUser, "Type \2/msg %s OHELP LOGIN\2 for more information.", s_StatServ);
		return;
	}
	...
	if (IS_NOT_NULL(pass = strtok(NULL, " "))) {
		...
		if (str_len(nick) > NICKMAX) {
			...
			send_notice_to_user(s_StatServ, callerUser, "Nicknames may not be longer than \2%d\2 characters.", NICKMAX);
			return;
		}
		else if (str_len(pass) > PASSMAX) {

			send_notice_to_user(s_StatServ, callerUser, "Passwords may not be longer than \2%d\2 characters.", PASSMAX);
			return;
		}
	}
	else {
...
static void do_stats(CSTR source, User *callerUser, ServiceCommandData *data) {
	...
	TRACE_MAIN_FCLT(FACILITY_STATSERV_HANDLE_STATS);

	send_notice_to_user(s_StatServ, callerUser, "*** \2Statistic Services Stats\2 ***");

	/* Send options. */
	APPEND_BUFFER(CONF_SET_DEBUG, "Debug")
	APPEND_BUFFER(CONF_SET_READONLY, "Read-Only")
	APPEND_BUFFER(CONF_SET_SNOOP, "Snooping")
	APPEND_BUFFER(CONF_SET_NOEXPIRE, "No-Expire")

	send_notice_to_user(s_StatServ, callerUser, "Options: %s", (len > 0) ? buffer : "None");

	/* Send uptime. */
	send_notice_to_user(s_StatServ, callerUser, "Stats up %s.", convert_time(buffer, sizeof(buffer), (NOW - start_time), LANG_DEFAULT));

	send_notice_to_user(s_StatServ, callerUser, s_SPACE);
	send_notice_to_user(s_StatServ, callerUser, "*** \2End of Stats\2 ***");
}
