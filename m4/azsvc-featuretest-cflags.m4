...
AC_DEFUN([AZSVC_FEATURETEST_CFLAGS], [
    ...
    AS_CASE(["x${enable_async_unwind_tables}"], [xno], [], [xyes], [
        AZSVC_TEST_CC_FLAGS([-fasynchronous-unwind-tables])
    ...
    AS_CASE(["x${enable_debugging_symbols}"], [xno], [], [xyes], [
        AZSVC_TEST_CC_FLAGS([-gdwarf-4])
        AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
            AZSVC_TEST_CC_FLAGS([-ggdb3])
            AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
                AZSVC_TEST_CC_FLAGS([-g])
                AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
    ...
    AS_CASE(["x${enable_stack_clash_protection}"], [xno], [], [xyes], [
        AZSVC_TEST_CC_FLAGS([-fstack-clash-protection])
        AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
    ...
    AS_CASE(["x${enable_stack_protector}"], [xno], [], [xyes], [
        AZSVC_TEST_CC_FLAGS([-fstack-protector-all])
        AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
            AZSVC_TEST_CC_FLAGS([-fstack-protector-strong])
            AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
                AZSVC_TEST_CC_FLAGS([-fstack-protector])
                AS_IF([test "${AZSVC_TEST_CC_FLAGS_RESULT}" = "no"], [
