...
* list.h
* 
* Based on IRC Services (c) 1996-2002 Andrew Church (achurch@achurch.org)
* 
*/
...
#undef  LIST_SEARCH_UNSORTED
#define LIST_SEARCH_UNSORTED(list, field, target, compare_function, result) \
    do { \
		int	i;	\
		\
		LIST_FOREACH (result, list) { \
			\
			i = compare_function(result->field, target); \
			\
			if (i == 0) \
				break; \
		} \
...
#undef  CREATE_HASHTABLE_UNSORTED
#define CREATE_HASHTABLE_UNSORTED(basename, datatype, hashsize)		CREATE_HASHTABLE_NOTAIL((basename), (datatype), (hashsize))
...
#undef  CREATE_HASH_ADD_UNSORTED
#define CREATE_HASH_ADD_UNSORTED(basename, datatype, key) \
	HASH_FUNCTIONS_MODIFIER void hash_##basename##_add(datatype *node) { \
		\
		datatype *branch = hashtable_##basename[HASH_HASHFUNC(node->key)]; \
		LIST_INSERT(node, branch); \
	}
...
#undef  CREATE_HASH_REMOVE_UNSORTED
#define CREATE_HASH_REMOVE_UNSORTED(basename, datatype, key)	CREATE_HASH_REMOVE_NOTAIL((basename), (datatype), (key))
...
#undef  CREATE_HASH_FIND_UNSORTED
#define CREATE_HASH_FIND_UNSORTED(basename, datatype, key) \
	HASH_FUNCTIONS_MODIFIER datatype *hash_##basename##_find(const char *value) { \
		\
		datatype	*result; \
		\
		LIST_SEARCH_UNSORTED(hashtable_##basename[HASH_HASHFUNC(value)], key, value, str_compare_nocase, result); \
		\
		return result; \
    }
